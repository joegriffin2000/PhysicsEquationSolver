...
def velocity1(d:float=None,t:float=None,v:float=None,verbose:bool=False,packed_vals:list=None) -> float:
    ...
    if packed_vals is not None and isinstance(packed_vals,list) and len(packed_vals) == 3:
        d = packed_vals[0]
        t = packed_vals[1]
        v = packed_vals[2]
    else:
...
def velocity2(vi:float=None,a:float=None,t:float=None,vf:float=None,verbose:bool=False,packed_vals:list=None)-> float:
    ...
    if packed_vals is not None and isinstance(packed_vals,list) and len(packed_vals) == 4: 
        vi = packed_vals[0]
        a = packed_vals[1]
        t = packed_vals[2]
        vf = packed_vals[3]
...
def kinetic_energy(m:float=None,v:float=None,ke:float=None,verbose:bool=False,packed_vals:list=None)-> float:
    ...
    try:
        ...
        raise ValueError(f"Non Numerics Passed as Values (m,v,ke):({m},{v},{ke})") if verbose else ValueError("Non Numerics Passed as Values")
    ...
    if ke is not None: 
        if m is None: 
            ...
            raise ValueError(f"No Unknowns Passed (m,v,ke):({m},{v},{ke})") if verbose else ValueError("No Unknowns Passed")
    else: 
        if m is None or v is None:
            raise ValueError(f"Too Many Unknowns (m,v,ke):({m},{v},{ke})") if verbose else ValueError("Too Many Unknowns")
        result = m * (v**2) * 0.5
